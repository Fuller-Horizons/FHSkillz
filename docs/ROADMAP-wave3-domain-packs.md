# Wave 3 roadmap — domain packs (planned, not shipped)

Wave 1 (0.18.0) shipped the reasoning kernel + frameworks pack. These
candidates from the 2026-07 consolidation are **staged, not built** — each
enters through jail-skill-miner's pipeline (mine → 4-box filter → dedup →
approve → author → jail-rate-skill ≥8.0) so the plugin grows deliberately, not by
accumulation. Every domain pack calls Layers 1–2 by name; none re-implements
kernel logic.

## Consulting & governance pack
- **client-engagement-architect** — advisory engagement structure: objective,
  scope + exclusions, decision rights, deliverables/acceptance, pricing,
  liability boundaries, CPRs, change control.
- **advisory-boundary-guard** — detect advisory→implementation drift:
  independence, authorization limits, reliance, decision ownership.
- **board-governance-reset** — disengaged-board diagnosis → expectations,
  accountability, reset communication + agenda (chains jail-cpr).
- **framework-fit-selector** — NIST CSF / CIS / 800-171+CMMC / ISO 27001 /
  SOC 2 / COBIT / ATT&CK / CCM / PCI selection driven by the business driver,
  with why-not for the runners-up.
- **cmmc-business-choice-architect** — CMMC as an executive business decision
  (exit / delay-and-accept-risk / lead), tied to revenue eligibility and
  assessor timelines.

## Brand & web pack
- **brand-truth-and-claim-auditor** — is the positioning *true*: factually
  supportable, honest to demonstrated capability, free of invented authority.
- **premium-experience-auditor** — hierarchy/spacing/typography/trust/CRO/
  a11y/mobile sweep through the three lenses (missing / unrealistic / how it
  fails despite good intentions).
- **commercial-website-strategy-pipeline** — strategy → research →
  positioning → messaging → IA → UX/CRO → design system → build → QA/security
  → analytics; blocks design-before-strategy.

## Build & systems pack
- **software-build-governor** — define job → freeze architecture → spec →
  bootstrap → implement → test → security review → intentional commit →
  preview → production → verify/document; per-phase tool permissions.
- **authoring-runtime-separation-architect** — what belongs in Notion vs
  DB vs source control vs runtime; authoring surfaces never silently become
  systems of record.
- ~~model-validation-council~~ — **absorbed into `jail-council` (shipped 0.20.0)**: the council protocol plus its when-to-convene framing.
- **capability-sandbox-evaluator** — evaluate a proposed tool/model/repo in a
  restricted sandbox with synthetic data before adoption (extends jail-rate-skill
  + jail-approval-gate).

## Growth pack
- **prospect-partner-fit-classifier** — ICP → candidates → classify
  (client/partner/referral/channel/competitor/vendor/irrelevant) → sampled
  review → selective enrich → approved export (extends
  jail-prospect).
- **dual-value-synergy-matcher** — two-sided partnership fit: needs,
  capabilities, buyer, timing, values, power balance, why each side says yes
  — and why either says no.
- **authority-content-engine** — assumption-challenging question →
  evidence-backed executive content → platform variants without diluting the
  argument.
- **meeting-to-execution-converter** — transcript/notes → decisions made/
  deferred, contradictions, actions with owners/deadlines, follow-ups, next
  agenda (the post-meeting twin of jail-cpr).
- **entity-structure-decision-advisor** — DBA / LLC / corp / subsidiary
  analysis across liability, tax burden, brand architecture, investor path —
  explicitly separated from licensed legal/tax advice.

## Also staged (mined, not yet absorbed)
- **jail-vocab-sync** — one controlled vocabulary, three enforcement points
  (module + migration + glossary), changed together.
- **jail-resume-ledger** — standalone checkpoint/resume discipline (currently
  embedded in jail-orchestrate; split out if non-orchestration demand shows).

## Entry bar (unchanged)
4-box filter · NEW-not-EXTENDS justified · code-free core (runnable parts →
jail-py-*) · router description with negative triggers · Gotchas · evals
seeded · jail-rate-skill ≥ 8.0 before the version bump ships it.
