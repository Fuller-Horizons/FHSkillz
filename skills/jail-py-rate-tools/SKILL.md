...
description: >-
  ...
  rate-skill skill. Use when rate-skill emits a JSON rating record, or when the
...
1. rate-skill emits the JSON record (schema in its `references/examples.md`).
...
- **Where code can't run**, fall back to rate-skill's manual checks (recompute
